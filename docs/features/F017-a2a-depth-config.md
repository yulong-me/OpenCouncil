...
当前 `MAX_A2A_DEPTH = 4` 硬编码在 `A2ARouter.ts` 里，所有房间统一使用，无法根据 Team 调整：
- 简单讨论（2-3人）：3层足够，过深容易跑题
- 深度辩论/复杂分析：10层+ 才能充分展开
- 无限制：某些复杂议题需要自由展开

用户需要按 Team 灵活调整，而不是"所有房间一个深度"。
...
- Hover 显示中文标签
- 点击即切换，实时生效
- 若此前设置过 room override，支持切回「跟随 TeamVersion」恢复默认值
...
```
┌─────────────────────────────────────────────────┐
│  TeamVersion.maxA2ADepth (默认)                    │
│  ├── roundtable-forum: 5                          │
│  └── software-development: 5                      │
├─────────────────────────────────────────────────┤
│  DiscussionRoom.maxA2ADepth (room 覆盖)            │
│  ├── null → 继承 TeamVersion 默认                 │
│  ├── 3/5/10/0 → room 覆盖                        │
...
```
effectiveDepth = room.maxA2ADepth ?? teamVersion.maxA2ADepth ?? 5
...
### DB Schema
- `rooms.max_a2a_depth INTEGER` (nullable, null=继承 TeamVersion)
- 迁移：新增列（nullable，不破坏现有数据）

### Backend
- `DiscussionRoom` interface + `maxA2ADepth`
- `TeamVersionConfig` interface + `maxA2ADepth` (seed 默认值)
- `A2ARouter.ts`: 读取 `room.maxA2ADepth ?? teamVersion.maxA2ADepth ?? 5`
- `stateMachine.ts`: 同上
...
- [x] AC-5: 无限制模式（∞）取消深度检查
- [x] AC-6: null 继承 TeamVersion 默认值
...
**症状**：在输入框打字时感到明显卡顿，IME（拼音输入法）输入时尤为严重。
...
1. 对 `findActiveMentionTrigger` + `open/closeMentionPicker` 加 **150ms debounce**，IME 输入只在组合完成（commit）时触发
