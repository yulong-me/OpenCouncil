...
**样式**：紧凑的 Segmented Control / Chip Toggle
- 当前深度高亮显示
- Hover 显示中文标签
- 点击即切换，实时生效
...
- [ ] AC-1: 房间顶栏显示当前深度（默认"中 (5)"）
- [ ] AC-2: 点击切换即时生效，后续 A2A 路由使用新深度
- [ ] AC-3: 深度到达上限时，agent 停止 @提及新专家，并提示"已达到协作深度上限"
- [ ] AC-4: 历史消息不重新计算（不回朔）
- [ ] AC-5: 无限制模式（∞）取消深度检查
- [ ] AC-6: null 继承 scene 默认值（向后兼容）
