---
feature_ids: [F018]
related_features: [F010, F012, F013, F052]
...
- 2026-04-20: 验证 topic 保留、运行时 prompt 上下文、软件开发 Team 默认工作流与空房间起手 UX，收口 feature 生命周期

## Why

用户创建房间后，真正的主流程是"明确收件人 → 对话 → 专家协作 → 验证/收敛"。当前系统已经实现了强制 @ 路由和内置 Team，但体验有三个断点：

- 创建房间时填写的 topic 没有进入后端 Room，房间上下文被弱化。
- 空房间和未 @ 发送时，只隐式打开 mention picker，用户不知道为什么不能发。
- 软件开发 Team 工作流偏 review 门禁，没有把"先理解需求、再规划、TDD/验证、证据、协作出口"变成主流程协议。
...
3. Team 工作流层：`team_versions.workflow_prompt` 注入每次专家执行，受 seed-once 保护。
4. Action 基础层：`stateMachine.ts` 构造当前执行者、角色和用户任务。
5. Runtime 上下文层：`teamPromptBuilder.ts` 注入工作目录、议题、任务、调用链、对话记录。
...
3. Mode/Guide 层：把模式引导和工作流触发从通用 system prompt 里拆出来。
4. Context Assembly：按 token 预算拼接历史、长期记忆和当前任务，避免无边界堆上下文。

### 可借鉴点

- 每次调用必须有"当前接收人"和"参与专家名单"，让模型知道自己是谁、队友是谁、何时 @。
- Team 工作流只放稳定协议，运行时上下文放动态事实。
- 软件开发 Team 应把工作流门禁写成顺序协议，而不是只写 review 态度。
...
- 软件开发内置 Team 强化为"理解需求 → 实施计划 → TDD/验证 → review/合入门禁 → 协作出口检查"。
- `teamPromptBuilder` 注入当前接收人和参与专家列表。
- 内置 Team 默认值只影响新 seed，不覆盖已有用户编辑过的 builtin Team。

## Acceptance Criteria

- [x] AC-1: `POST /api/rooms` 使用用户提供的 trimmed topic。
- [x] AC-2: 运行时 prompt 包含 `【当前接收人】` 和 `【参与专家】`。
- [x] AC-3: 软件开发 Team 默认工作流包含理解需求、实施计划、TDD、review、测试证据。
...
- 内置 Team 工作流由 `backend/src/prompts/builtinTeams.ts` 提供，迁移层只负责 schema。
- `teamsRepo.ensureBuiltinTeams()` 保持 seed-once 语义。
- 软件开发 prompt 借鉴 clowder-ai 的 invocation anchor 思路，但不引入复杂 guide / queue 系统。

## Verification

- `pnpm --dir backend exec vitest run tests/rooms.http.test.ts tests/teams.test.ts tests/builtinTeams.test.ts`
