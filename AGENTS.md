...
- **Public local defaults**: use frontend 7002 and API 7001 to avoid colliding with another local runtime.
...
系统内置的 Providers、Teams、Agents 在**首次启动时**一次性 seed，之后系统不再自动覆盖用户修改过的数据。
...
| Teams | `backend/src/db/repositories/teams.ts` `ensureBuiltinTeams()` | `teamsRepo.ensureBuiltinTeams()` |
| Agents | `.agents/skills/{id}-perspective/SKILL.md` | `agentsRepo.upsert()` 补充缺失项 |
| Shared Rules | `cat-cafe-skills/refs/shared-rules.md` | （后续通过 `teamPromptBuilder.ts` 注入，本期仅文档占位） |

### Seed 机制（`app_meta.bootstrap_seed_version`）

| DB 状态 | 行为 |
|---------|------|
| 干净 DB（agents/providers/teams 均为空） | seed 所有内置数据，写入 `bootstrap_seed_version=1` |
...
- **Teams**：`ensureBuiltinTeams()` — 已存在的 builtin Team 不被覆盖，用户可自由编辑 Team workflow prompt
- **Agents**：仅插入缺失的内置 agent，已有 agent（含用户修改的 systemPrompt）保留

### 升级

老用户升级新版本：不会清空/覆盖已有 agents、providers、teams；内置 Team 只在完全不存在时才插入。
