...
assert.match(roomHeader, /gap-1 sm:gap-2/)
assert.match(roomHeader, /md:hidden inline-flex h-9 w-9/)
assert.match(depthSwitcher, /aria-expanded=\{open\}/)
assert.match(depthSwitcher, /inline-flex h-9 items-center/)
assert.match(depthSwitcher, /document\.addEventListener\('pointerdown', handleOutsideInteraction\)/)
assert.match(depthSwitcher, /document\.addEventListener\('click', handleOutsideInteraction\)/)
assert.match(depthSwitcher, /document\.addEventListener\('keydown', handleKeyDown\)/)
assert.match(depthSwitcher, /aria-pressed=\{value === option\.value\}/)
assert.doesNotMatch(depthSwitcher, /role="menu"/)
assert.doesNotMatch(depthSwitcher, /role="menuitemradio"/)
assert.match(depthSwitcher, /min-h-9 w-full/)
